...
sra_file = arguments.sra_file
sketch_db_ref = arguments.path_db

OUTPUT_DIR = "sra_test_downloads"
LOG_FILE = "processed_sra.log"

#this will open and log the SRA it script will be processing
processed_sra = set()
...
while True:
    ...
    os.system(f"sylph sketch -c 70 -1 {fastq_1} -2 {fastq_2} -d {sra_dir}")
    os.system(f"sylph profile --min-number-kmers 3 >> results.tsv {sketch_db_ref} {fastq_1 + '.paired.sylsp'}")
